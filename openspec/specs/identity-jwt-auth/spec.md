...
Defines the authentication model for LitXusCount: ASP.NET Core Identity for user storage and JWT bearer tokens (with refresh tokens) for API-only authentication, with no cookie or server-rendered login flow.
