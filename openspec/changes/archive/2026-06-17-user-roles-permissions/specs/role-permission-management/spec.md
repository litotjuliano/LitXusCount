...
The system SHALL define a fixed, compile-time catalog of permissions (e.g. `Users.View`, `Users.Create`, `Users.Edit`, `Users.Delete`, `Roles.View`, `Roles.Create`, `Roles.Edit`, `Roles.Delete`). Permissions SHALL be assignable to roles as Identity role claims (type `"permission"`), with no per-user permission overrides — a user's effective permissions are the union of their assigned roles' permissions.
