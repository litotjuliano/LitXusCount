...
import type { AccExpenseItem } from "../../api/accounts/expenses";
...
const emptyForm: FormState = { accAccountId: "", name: "", expenseDate: today(), amount: "", note: "" };

const ExpensesLayer = () => {
  const { pagedQuery, accountsQuery, createMutation, deleteMutation } = useExpenses();
  ...
  const canCreate = hasPermission(Permissions.Accounts.Expense.Create);
  const canDelete = hasPermission(Permissions.Accounts.Expense.Delete);
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    createMutation.mutate(
      { accAccountId: Number(form.accAccountId), name: form.name.trim(), expenseDate: form.expenseDate, amount: parseFloat(form.amount), note: form.note.trim() || null },
    ...
    <>
      <Modal show={showModal} onHide={closeModal} centered>
        <Modal.Header closeButton>
          <Modal.Title className='h6'>Record Expense</Modal.Title>
        </Modal.Header>
        <form id='expense-form' onSubmit={handleSubmit}>
          <Modal.Body>
            <div className='row gy-3'>
              ...
              <div className='col-12'>
                <label className='form-label'>Expense Name <span className='text-danger'>*</span></label>
                <input type='text' className='form-control' value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required maxLength={128} />
              </div>
          ...
          <Modal.Footer>
            ...
            <button type='submit' form='expense-form' className='btn btn-primary' disabled={createMutation.isPending}>Add Expense</button>
          </Modal.Footer>
        </form>
      </Modal>

      <Modal show={pendingDeleteId !== null} onHide={() => setPendingDeleteId(null)} centered size='sm'>
        <Modal.Body>This will reverse the expense and restore the account balance. Continue?</Modal.Body>
        <Modal.Footer>
      ...
      <div className='row'>
        <div className='col-12'>
          <PaginatedTable
            ...
            renderRow={(item: AccExpenseItem) => (
              <tr key={item.id}>
                <td>{item.accountName}</td>
                <td>{item.name}</td>
                <td>{item.expenseDate.slice(0, 10)}</td>
