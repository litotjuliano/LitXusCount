...
[ApiController]
[Authorize]
[Route("api/settings/lhdn-currency-codes")]
public class LhdnCurrencyCodesController(ApplicationDbContext db) : ControllerBase
{
    [HttpGet]
    [Authorize(Policy = "Permission." + Permissions.Settings.LhdnCurrencyCode.View)]
    public async Task<IActionResult> List([FromQuery] PagedQuery query, CancellationToken ct)
    {
        var q = db.LhdnCurrencyCodes.Where(x => x.IsActive);
    ...
    {
        var items = await db.LhdnCurrencyCodes
            .Where(x => x.IsActive)
            .OrderBy(x => x.Code)
