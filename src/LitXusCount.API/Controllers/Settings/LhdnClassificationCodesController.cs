...
[ApiController]
[Authorize]
[Route("api/settings/lhdn-classification-codes")]
public class LhdnClassificationCodesController(ApplicationDbContext db) : ControllerBase
{
    [HttpGet]
    [Authorize(Policy = "Permission." + Permissions.Settings.LhdnClassificationCode.View)]
    public async Task<IActionResult> List([FromQuery] PagedQuery query, CancellationToken ct)
    {
        var q = db.LhdnClassificationCodes.Where(x => x.IsActive);

        if (!string.IsNullOrWhiteSpace(query.Search))
        {
            var term = query.Search.Trim().ToLower();
            q = q.Where(x => x.Code.ToLower().Contains(term) || x.Description.ToLower().Contains(term));
        }
    ...
    {
        var items = await db.LhdnClassificationCodes
            .Where(x => x.IsActive)
            .OrderBy(x => x.Code)
