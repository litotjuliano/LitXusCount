...
[ApiController]
[Authorize]
[Route("api/settings/lhdn-countries")]
public class LhdnCountriesController(ApplicationDbContext db) : ControllerBase
{
    [HttpGet]
    [Authorize(Policy = "Permission." + Permissions.Settings.LhdnCountry.View)]
    public async Task<IActionResult> List([FromQuery] PagedQuery query, CancellationToken ct)
    {
        var q = db.LhdnCountries.Where(x => x.IsActive);
    ...
    {
        var items = await db.LhdnCountries
            .Where(x => x.IsActive)
            .OrderBy(x => x.Code)
