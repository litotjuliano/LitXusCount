...
[ApiController]
[Authorize]
[Route("api/settings/payment-codes")]
public class PaymentCodesController(ApplicationDbContext db) : ControllerBase
{
    [HttpGet]
    [Authorize(Policy = "Permission." + Permissions.Settings.PaymentCode.View)]
    public async Task<IActionResult> List([FromQuery] PagedQuery query, CancellationToken ct)
    {
        var q = db.PaymentCodes.Where(x => x.IsActive);
    ...
    {
        var items = await db.PaymentCodes
            .Where(x => x.IsActive)
            .OrderBy(x => x.Code)
